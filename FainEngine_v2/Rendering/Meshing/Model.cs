...
public class Model : IDisposable
{
    ...
    private unsafe IMesh ProcessMesh(AssimpMesh* mesh, Scene* scene)
    {
        // data to fill
        ...
        var result = new AMesh<Vertex, uint>();
        result.SetData(vertices, indices);
        return result;
    }
    ...
    private unsafe List<Texture> LoadMaterialTextures(Material* mat, TextureType type, string typeName)
    {
        ...
        /*
        var textureCount = _assimp.GetMaterialTextureCount(_material, type);
        List<Texture> textures = new List<Texture>();
        for (uint i = 0; i < textureCount; i++)
        {
            AssimpString path;
            _assimp.GetMaterialTexture(_material, type, i, &path, null, null, null, null, null, null);
            bool skip = false;
